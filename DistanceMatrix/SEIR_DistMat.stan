...
functions {
	real[] SEIR(real t, 
		...
		real pi_ij_Vec[n_region*n_region];  // a vector for the distance between the regions: a symmetric matrix put inot a vector
		...
		pi_ij_Vec = x_r[(5*n_region+5):((5*n_region+4) + (n_region*n_region))];   	 // 169 elements
		TS_Mob_Vec = x_r[((5*n_region+4) + (n_region*n_region) + 1):(30*n_region + ((5*n_region+4) + (n_region*n_region)))];
		...
		pi_ij = to_row_vector(pi_ij_Vec);
		Mobility_Vec = to_row_vector(TS_Mob_Vec);
		
		// constructing the probability of movement
		// f_ij = 1 ./ (rep_row_vector(1,(n_region*n_region)) + to_row_vector(D_Vec)); // stan does not have element wise operator for raising to the power. So I replace k with 1 directly
...
data {
	...
	// Fixed parameters
	  real y0_vars[4*n_region];  
		real Init_Suscept[n_region];       // initial susceptible population
		real pi_ij_Vec[n_region*n_region]; 
...
transformed data {
	real x_r[((5*n_region+4) + (n_region*n_region)) + (30*n_region)];
	int x_i[n_region + 1];
	...
	x_r[(5*n_region+5):((5*n_region+4) + (n_region*n_region))] = pi_ij_Vec;   	 // 169 elements
	x_r[((5*n_region+4) + (n_region*n_region) + 1):(30*n_region + ((5*n_region+4) + (n_region*n_region)))] = TS_Mob_Vec;
}
